...
/**
 * Convert the library's display tokens (e.g. `DD/MM/YYYY`) to the internal format used by the
 * date engine (`dd/MM/yyyy`). Only year (`Y`) and day-of-month (`D`) differ in case.
 */
...
/** Format a date using the library's display tokens. Returns `''` for nullish/invalid input. */
...
/** Parse a string into a `Date` using the library's display tokens. Returns `null` when invalid. */
